...
As a customer, I want my call to be taken quickly with little waiting time in order to save time. | High | 9 | The average waiting time in que for a customer has decreased significantly compared to the previous system.
...
As a relationship manager, I want to receive customer details with guidelines and a script, so I can provide an improved service to end-customers for outbound calls. | High | 7 | Relationship managers are provided with customer details, guidelines and a script which ultimately improves potential sales and customer service with outbound calls.
As a relationship manager, I want to automatically receive a customer target list, so I can produce sales more effectively and efficiently to targeted customers. | High | 7 | Relationship managers automatically receive a target list based on their skills and profile which ultimately makes relationship managers more convincing and increases the chance of a sale.
